...
int main(char argc, char *argv[]){
    ...
    // Initialize muetexes and queues
    for(int i = 0; i < QUEUE_NUMBER; i++){
        pthread_mutex_init(&queues[i].mutex, &mutex_attr);
...
void enqueue(struct PriorityQueue *queue, struct parsedMessage request){
    // printf("Enqueueing request in %d queue\n", request.number);
    pthread_mutex_trylock(&queue->mutex);
    // int ret = pthread_mutex_trylock(&queue->mutex);
    // if(ret != 0){
    //     printf("ret enqueue: %d\n", ret);
    //     perror("Trying to lock mutex");
    // }
