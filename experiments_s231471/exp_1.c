...
uint64_t reverse_hash(uint64_t start, uint64_t end, unsigned char *hash_message){
    ...
    while (counter > 1) {
        ...
        if (isEqual) {
            // printf("Hellow2\n");
            // printf("Hash existing in memory!");
            return head->answer;
            break;
        } else {
            if (head->second_chance) {
                ...
                frames[9] = head;
                // printf("moved to the back!, check last secondchancevalue:\n");
            } else {
                // printf("Hellow4\n");
                //TODO: remove and add new block to the end
                for (int i=1; i<counter; i++) {
                    frames[i - 1] = frames[i];
                }
                counter--;
                // printf("Something popped\n");
                break;
            }
        }
        break;
    ...
    for(test_num = start; test_num < end; test_num++){
        ...
        if(same){
            // printf("Current count in list is : %d\n", counter);
            // Add the first 10 elements into the SecondChance array
...
struct parsedMessage dequeue(struct PriorityQueue *queue) {
    ...
    // Shell sort to maintain the priority order (highest priority at the top)
    for (int gap = queue->size; gap > 0; gap /= 2) {
			for (int i = gap/2; i < queue->size; i++) {
				struct parsedMessage tempElement = queue->elements[i];
				int j;
				for (j = i; j >= gap && queue->elements[j-gap].priority > tempElement.priority; j-=gap){
						queue->elements[j] = queue->elements[j-gap];
				}
				queue->elements[j] = tempElement;
			}
	}
    return maxElement;
}
